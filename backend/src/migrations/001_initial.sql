...
INSERT INTO products (collection_id, story_id, name, slug, description, price, original_price, sale_price, stock, sku, images, sizes, status, category, gender, badge, badge_class, story_text, offer_name)
SELECT
  c.id,
  s.id,
  p.name, p.slug, p.description, p.price, p.original_price, p.sale_price, p.stock, p.sku, p.images, p.sizes, p.status, p.category, p.gender, p.badge, p.badge_class, p.story_text, p.offer_name
FROM (VALUES
  ('night-ride', 'midnight-drift', 'Midnight Drift', 'obsidian-lava-tee', 'A signature heavyweight black t-shirt with structured chest layout, boxy drop-shoulder fit, and high-density print detail.', 899, 1499, NULL::INTEGER, 50, 'YARO-001', ARRAY['images/detail_pics/center_model.png','images/detail_pics/left_back.png','images/detail_pics/right_front.png'], ARRAY['S','M','L','XL','XXL'], 'active', 'Night Ride', 'Any', 'Chapter', 'bestseller', 'The quiet roads know every dream that never reached daylight.', 'Flash Sale'),
  ('night-ride', 'city-lights', 'City Lights', 'violet-haze-hoodie', 'Heavyweight combed ringspun cotton with drop-shoulder silhouette, massive structured hood, and relaxed cuffs.', 899, 1499, NULL::INTEGER, 40, 'YARO-002', ARRAY['images/violet_haze_hoodie.png'], ARRAY['S','M','L','XL','XXL'], 'active', 'Night Ride', 'Any', 'Chapter', 'bestseller', 'Lost in the neon glow, chasing shadows of a life we used to know.', 'Flash Sale'),
  ('night-ride', 'empty-roads', 'Empty Roads', 'shadow-cargo-pant', 'Technical shadow cargo pant engineered with heavy nylon cotton canvas, deep modular pockets, dual side utility straps, and matte steel adjustment hardware.', 899, 1499, NULL::INTEGER, 30, 'YARO-003', ARRAY['images/shadow_cargo_pant.png'], ARRAY['S','M','L','XL','XXL'], 'active', 'Night Ride', 'His', 'New', 'new', 'No destination, just the cold wind and the open path ahead.', 'Season Sale'),
  ('urban-echo', 'concrete-soul', 'Concrete Soul', 'crop-lacroix-tee', 'Minimalist off-white crop tee crafted with organic soft rib cotton, structured box fit drape, and double stitch hem details.', 899, 1399, NULL::INTEGER, 45, 'YARO-004', ARRAY['images/crop_lacroix_tee.png'], ARRAY['S','M','L','XL','XXL'], 'active', 'Urban Echo', 'Hers', 'Chapter', 'bestseller', 'Born of the pavement, thriving in the noise of the crowded street.', 'Flash Sale'),
  ('urban-echo', 'lost-signal', 'Lost Signal', 'relaxed-fit-sweatshirt', 'Cozy oversized charcoal crewneck sweatshirt with organic cotton fleece backing, drop-shoulder seams, and rib finish mockneck collar.', 899, 1499, NULL::INTEGER, 35, 'YARO-005', ARRAY['images/relaxed_fit_sweatshirt.png'], ARRAY['S','M','L','XL','XXL'], 'active', 'Urban Echo', 'Any', 'Chapter', 'bestseller', 'A frequency cut short, a voice fading into static.', 'Flash Sale'),
  ('urban-echo', 'silent-crowd', 'Silent Crowd', 'oversized-drop-shoulder', 'Heavyweight sand beige drop-shoulder t-shirt with premium mock neck rib collar and structured boxy aesthetic outline.', 899, 1499, NULL::INTEGER, 35, 'YARO-006', ARRAY['images/oversized_drop_shoulder.png'], ARRAY['S','M','L','XL','XXL'], 'active', 'Urban Echo', 'Any', 'New', 'new', 'Surrounded by thousands, yet walking in absolute silence.', 'Season Sale'),
  ('midnight-society', 'late-escape', 'Late Escape', 'mini-logo-tee', 'Classic kids white graphic t-shirt in extremely soft organic ring-spun combed cotton, featuring the YARO micro logo on the chest.', 899, 1299, NULL::INTEGER, 25, 'YARO-007', ARRAY['images/mini_logo_tee.png'], ARRAY['XS','S','M','L'], 'active', 'Midnight Society', 'Mini-Gent', 'New', 'new', 'Sneaking out into the cool night, leaving the city behind.', 'Clearance'),
  ('wander', 'lost-path', 'Lost Path', 'mini-hoodie', 'Ultra soft double fleece organic light beige kid hoodie. Minimalist clean style with micro branding and perfect warmth for children.', 899, 1399, NULL::INTEGER, 20, 'YARO-008', ARRAY['images/mini_hoodie.png'], ARRAY['XS','S','M','L'], 'active', 'Wander', 'Mini-Lady', 'New', 'new', 'Not all who wander are looking for a way home.', 'Clearance')
) AS p(collection_slug, story_slug, name, slug, description, price, original_price, sale_price, stock, sku, images, sizes, status, category, gender, badge, badge_class, story_text, offer_name)
